...
public class MakeUpdate implements Runnable {
	...
	public void run() {
		...
		try {
			...
			MyNotify.addNotification(context,
					MyNotification.DATENBANK_AKTUALISIEREN);
			liste = DB.aktualisieren_datenbank(context);
			MyNotify.removeNotification(context,
					MyNotification.DATENBANK_AKTUALISIEREN);
			if (!liste.isEmpty()) {
				MyNotify.addNotification(context,
						MyNotification.DATENBANK_NEUE_FOLGEN_GEFUNDEN, liste);
			}
			if (!tStarted) {
